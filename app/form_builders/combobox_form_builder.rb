class ComboboxFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def combobox_tag(name, options, opts= {})
     @template.content_tag(:input, :name => name, :id => sanitize_to_id(name), :type => "text", :list => opts[:list_id], :onkeyup => 'inputChanged("' + opts[:form_selector] + '", "' + opts[:input_selector] + '")') do
